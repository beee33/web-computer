...
function shuffle_list(array_in) {
  ...
  for (var index = 0; index < len_to_get; index++) {
    newer_list.push(
      new_list.splice(Math.floor((len_to_get - index) * Math.random()), 1)[0],
    );
  }
...
async function spawn_line(text, err) {
  ...
  for (const [index, element] of split_items.entries()) {
    ...
    if (split_items.length == 1) {
      ...
      node.innerHTML =
        "<pre><span style=white-space:preserve-spaces>" +
        element +
        "</span></pre>";
    }
...
function move_window_forward(browser_id) {
  ...
  if (browser_windows.size > 0) {
    if (
      browser_windows.get(browser_id)[1] >=
      browser_windows.get(browser_id)[0].length - 1
    ) {
      console.log("cant go forward");
    } else {
      browser_windows.get(browser_id)[1] += 1;
      browser.children[1].children[0].src =
        browser_windows.get(browser_id)[0][browser_windows.get(browser_id)[1]];
    }
  }
}
...
function search_web_browser(browser_id) {
  ...
  if (input_res.includes("https://")) {
    ...
    browser_windows
      .get(browser_id)[0]
      .splice(browser_windows.get(browser_id)[1], 0, input_res);
  ...
  switch (meme_image.split(".")[1]) {
    case "png":
      html_insert +=
        `<img id = \"meme` +
        meme_num +
        `\" style="max-width:100%;max-height:470px;center;" src="` +
        meme_image +
        `">`;
      break;
    case "mov":
      html_insert +=
        `<video  onloadstart=\"this.volume=` +
        audio_vol +
        `\"" id = \"meme` +
        meme_num +
        `\" style="max-width:100%;max-height:470px;" controls autoplay>
				<source src="` +
        meme_image +
        `" type="video/mp4">
				Cant play video :(
...
function make_ips() {
  ...
  fetch("bad_guys.txt")
    ...
    .then((text) => {
      ...
      for (ip of ip_range) {
        new_ip_range.splice(
          Math.floor(new_ip_range.length * Math.random()),
          0,
          ip,
        );
      }
      ...
      text_full +=
        `<div style="font-family: Monospace;font-size:30px;text-align: center;white-space: pre;margin-top:0px;margin-bottom:0px;">` +
        ip_range.length +
        ` tormented souls</di
v>`;
      function pretty_items(txt) {
        return (
          `<div style="font-family: Monospace;font-size:15px;white-space: pre;margin-top:0px;margin-bottom:0px;">` +
          txt +
          `</div>`
        );
      }
...
function make_beefetch() {
  ...
  var items_in = [
    "<span style=color:darkturquoise>guest</span>@<span style=color:darkturquoise>" +
      OS_NAME +
      "</span>",
    "-----------",
    ...
    "<span style=color:darkturquoise>Programs</span>: " +
      Object.keys(installed_programs).length +
      " (Desktop)",
    "<span style=color:darkturquoise>Shell</span>: bsh 5.8",
    "<span style=color:darkturquoise>Display</span>: " +
    ...
    "<span style=color:darkturquoise>Theme</span>: " + CHOOSEN_THEME_NAME,
  ...
  function make_new_line(item_at, textin) {
    ...
    return (
      ...
      new_logo[item_at] +
      "</span>" +
      " ".repeat(max_size - new_logo[item_at].length + margin) +
      textin
    );
  }
  var mob_adds = "";
  ...
  var new_text =
    `<div style="` +
    mob_adds +
    `;background-color:black;height:100%;color:white;">
		<span style="font-family: Monospace;font-size:15px;white-space: pre;margin-top:0px;margin-bottom:0px;"><span style="color:lawngreen;">guest@` +
    OS_NAME +
    `</span>:<span style="color:cornflowerblue;;">~</span>$ beefetch </span>`;
  for (var [index, item] of items_in.entries()) {
    var new_item = make_new_line(index, item);

    new_text +=
      `<p style="font-family: Monospace;font-size:15px;white-space: pre;margin-top:0px;margin-bottom:0px;"><span>` +
      new_item +
      "</span></p>";
  }
  new_text +=
    `<span style="font-family: Monospace;font-size:15px;white-space: pre;margin-top:0px;margin-bottom:0px;"><span style="color:lawngreen;">guest@` +
    OS_NAME +
    `</span>:<span style="color:cornflowerblue;;">~</span>$</span></div>`;
...
function make_web_browser() {
  ...
  var win_id = make_window(
	...
	<div id = ` +
      ...
      `px;">
		...
		<div style="height:` +
      (browser_height - box_top_height - border_width) +
      `px;width:` +
      browser_width +
      `px;">
			<iframe src= "` +
  ...
  if (!showed_warning) {
    showed_warning = true;
    make_warn(
      "This web browser is NOT a search engine and can only load URLs. For security only few websites can run here.",
    );
  }
}
...
function make_view(path, is_scale) {
  ...
  img.onload = function () {
    ...
    if (is_scale) {
      scaled_width =
        IMG_SCALE_SIZE * document.getElementById("background").offsetWidth;
      scaled_height =
    ...
    var win_id = make_window(
      ...
      path,
      `
			<img style="display:block;height:` +
        scaled_height +
        `px;"height = "` +
        scaled_height +
        `" width = "` +
...
function make_about() {
  ...
  html +=
    `<span style="font-size: larger;">Hello and welcome to ` +
    OS_NAME +
    `!</span>
        <div style="height:10px"></div>
        <div>
...
function make_random() {
  if (DISABLE_RANDOM_WARNING == false) {
    ...
    setTimeout(
      make_random,
      Math.floor(WARN_ONSET_DELAY + Math.random() * WARN_RANDOM),
    );
  }
}
...
function make_warn(text, sporadic) {
  ...
  var html =
    ...
    `px height=` +
    img_height +
    `px src="/desktop_icons/warn.png">
		</div>
		<div style="text-align: center;width: 100%;margin: auto;margin-left:` +
    (win_height - block_height) / 2 +
    `px">
			` +
    text +
...
function push_icons_to_desktop() {
  var icons_x = Math.floor(
    document.getElementById("background").offsetWidth / SET_ICON_WIDTH,
  );
  var icons_y = Math.floor(
    (document.getElementById("background").offsetHeight - taskbar_height) /
      SET_ICON_WIDTH,
  );
  ...
  document.getElementById("desktop").style =
  ...
  for (const icon_cur of Object.keys(installed_programs)) {
    if (installed_programs[icon_cur]["in_desktop"] != null) {
      ...
      if (!is_mobile) {
        icon.id =
          "icon_" +
          installed_programs[icon_cur]["in_desktop"][0] +
          "_" +
          installed_programs[icon_cur]["in_desktop"][1];
      }
      ...
      icon.innerHTML +=
        ...
        click_type +
        `=` +
        installed_programs[icon_cur]["function_param"] +
        ` ` +
        start_click_type +
        `= clicked_icon() style="user-select: none;width:100%;height:100%;">
				<div style="width:100%;height:` +
        (SET_ICON_WIDTH - 40) +
        `px;align-items: center;justify-content: center;display: flex;">
				<div>
...
function add_icons_to_task_bar(win_id, program_id) {
  if (!is_mobile) {
    ...
    var icon_margin = 4;
    var icon_height = parseInt(
      document
        .getElementById("task_bar")
        .style.height.substring(
          0,
          document.getElementById("task_bar").style.height.length - 2,
        ),
    );
    icon.innerHTML +=
      `
	<div onclick=move_to_front_window("` +
      win_id +
      `") style="width:` +
      document.getElementById("task_bar").style.height +
      `;height:` +
      document.getElementById("task_bar").style.height +
      `">
		<img draggable="false" src = "` +
      icon_path +
      `" style="user-select: none;margin:` +
      icon_margin +
      `px;width:` +
      (icon_height - icon_margin * 2) +
      `px;height:` +
      (icon_height - icon_margin * 2) +
      `px;">
		</img>
...
function load_dropper() {
  ...
  img.addEventListener("load", (e) => {
    ...
    for (let i = 0; i < data_len; i += 4) {
      ...
      if (avg > 200) {
        data[i] = current_theme.window_border_color[0];
        data[i + 1] = current_theme.window_border_color[1];
        data[i + 2] = current_theme.window_border_color[2];
      } else {
        data[i] = current_theme.window_title_text_color[0];
        data[i + 1] = current_theme.window_title_text_color[1];
        data[i + 2] = current_theme.window_title_text_color[2];
      }
    }
...
function mouse_move(e) {
  if (window_hold != null) {
    ...
    var win_width = parseInt(
      canvas_select.style.width.substring(
        0,
        canvas_select.style.width.length - 2,
      ),
    );
    var win_height = parseInt(
      canvas_select.style.height.substring(
        0,
        canvas_select.style.height.length - 2,
      ),
    );

    if (
      new_y >= 0 &&
      e.clientY + win_height - window_shift_y + taskbar_height <=
        parseInt(document.getElementById("background").offsetHeight)
    ) {
    ...
    if (
      new_x >= 0 &&
      e.clientX + win_width - window_shift_x <=
        parseInt(document.getElementById("background").offsetWidth)
    ) {
...
function load_theme(theme_name) {
  current_theme = desktop_theme_list[theme_name];
  load_dropper();
  CHOOSEN_THEME_NAME = theme_name;
  ...
  for (var element of document.getElementsByClassName("sec_color")) {
    element.style.backgroundColor =
      ...
      "," +
      current_theme.window_border_color[1] +
      "," +
  ...
  for (var element of document.getElementsByClassName("style_vol")) {
    element.style.background =
      ...
      "," +
      current_theme.window_title_text_color[1] +
      "," +
  ...
  for (var element of document.getElementsByClassName("main_color")) {
    element.style.backgroundColor =
      ...
      "," +
      current_theme.window_context_background_color[1] +
      "," +
...
function make_exit_bttn(canvas_id) {
  if (!is_mobile) {
    ...
    ctx.strokeStyle = "#" + rgb_to_hex(current_theme.window_border_color);
    ctx.lineCap = "square";
    ctx.stroke();
...
function make_theme_window() {
  ...
  if (is_mobile) {
    entrie_height =
      (document.getElementById("background").offsetHeight *
        ...
        help_height -
        border_width) /
      Object.keys(desktop_theme_list).length;
  }
  var win_height =
    Object.keys(desktop_theme_list).length * entrie_height +
    help_height +
    border_width;
  var font_size = ``;
  var font_size_about = ``;
  ...
  function spawn_select_window(theme, name) {
    return (
      `<div onclick=load_theme("` +
      name +
      `") style="height:` +
      entrie_height +
      `px;display: grid;grid-template-columns: auto ` +
      border_width +
      `px ` +
      text_icon_size +
      ` 30px 30px;">
      ...
      `;color:#` +
      rgb_to_hex(theme.window_context_text_color) +
      `;justify-content: center;align-items: center;text-align: center;grid-column:3;grid-row: 1;"><span style="` +
      font_size_sample +
      `">Text</span></div>
			<div style="grid-column:4;grid-row:1;background-color:#` +
      rgb_to_hex(theme.window_border_color) +
      `"></div>
			<div style="grid-column:5;grid-row:1;background-color:#` +
      rgb_to_hex(theme.window_title_text_color) +
      `"></div>
			</div>`
    );
  }
  ...
  for (const theme_cur of Object.keys(desktop_theme_list)) {
    inj_code += spawn_select_window(desktop_theme_list[theme_cur], theme_cur);
  }
  inj_code += `</div>`;
  var canvas_id = make_window(
    Math.ceil(
      document.getElementById("background").offsetWidth / 2 - win_width / 2,
    ),
    Math.ceil(
      document.getElementById("background").offsetHeight / 2 - win_height / 2,
    ),
    win_width,
    win_height,
    "Theme Selector",
...
async function load_navbar() {
  ...
  if (!is_mobile) {
    ...
    node6.style.right =
      parseInt(node.style.width.substring(0, node.style.width.length - 2)) +
      parseInt(node2.style.width.substring(0, node2.style.width.length - 2)) +
      "px";
  ...
  node3.innerHTML +=
    `
		<img draggable="false" src="` +
    OS_ART_LOGO +
    `" style="user-select: none;width:` +
    (taskbar_height - icon_margin * 2) +
    `px;margin:` +
    icon_margin +
    `px;height:` +
    (taskbar_height - icon_margin * 2) +
  ...
  if (!is_mobile) {
    ...
    node4.style.left =
      parseInt(node3.style.width.substring(0, node3.style.width.length - 2)) +
      10 +
      "px";
    node4.style.width =
      document.getElementById("background").offsetWidth -
...
function mouse_down_window(id) {
  ...
  window_shift_x =
    e.clientX -
    parseInt(
      canvas_select.style.left.substring(
        0,
        canvas_select.style.left.length - 2,
      ),
    );
  window_shift_y =
    e.clientY -
    parseInt(
      canvas_select.style.top.substring(0, canvas_select.style.top.length - 2),
    );
...
function make_window(
  ...
  if (!is_mobile) {
    if (x_pos == null) {
      x_pos =
        document.getElementById("background").offsetWidth / 2 -
        (screen_width + border_width * 2) / 2;
    }
    if (y_pos == null) {
      y_pos =
        document.getElementById("background").offsetHeight / 2 -
        (screen_height + box_top_height + border_width * 3) / 2;
    }
	...
	<div id="` +
      ...
      `px; height:` +
      (screen_height + box_top_height + border_width * 3) +
      `px; z-index:` +
      z_index +
      `;top:` +
      y_pos +
      ...
      x_pos +
      `px; position:fixed;">
		<div onmousedown=mouse_down_window("` +
      screen_id +
      `")>
			<div style=\"width:` +
      (screen_width - (box_top_height - border_width)) +
      `px;height:` +
      ...
      `px; height:` +
      screen_height +
      `px; grid-area: 1 / 2 / span 1 / span 1;">
		` +
      html +
      `
			</div>
			<div style=" grid-area: 1 / 3 / span 1 / span 1;width:` +
      border_width +
      `px;">

			</div>

			<div style=" grid-area: 2 / 1 / span 1 / span 3;height:` +
      border_width +
      `px;">
    ...
    node.innerHTML +=
      `
        <div id="` +
      screen_id +
      `" style="user-select: none;top: 0;width:` +
      document.getElementById("background").offsetWidth +
      `px; height:` +
      document.getElementById("background").offsetHeight +
      `px; z-index:` +
...
async function start_desktop() {
  ...
  if ((height_test <= 700 || width_test <= 1000) & !is_mobile) {
    warn_test =
      "Browser Window should be at least 1000 pixels wide and 700 pixels tall.<br> Current size is: " +
      width_test +
      "x" +
      height_test +
      " pixels";
