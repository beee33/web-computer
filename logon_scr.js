...
let circleMask;
function preload() {
  wallpaper_image = loadImage(current_theme.wallpaper_path);
...
function draw() {
  if (skip_start || is_mobile) {
    ...
    var main_color = color(current_theme.window_context_background_color);
    var sec_color = color(current_theme.window_border_color);
    var third_color = color(current_theme.window_title_text_color);
    ...
    rect(
      new_x + logon_width - box_top_height,
      new_y,
      box_top_height,
      box_top_height,
    );
    ...
    text(
      OS_NAME + " " + VER_NAME,
      new_x + border_width,
      new_y + border_width + box_top_height / 2,
    );

    var acc_image_x = new_x + 30;
    var acc_image_y =
      new_y + (logon_height + box_top_height) / 2 - logo_size / 2;
    ...
    fill(third_color);
    rect(
      acc_image_x + logo_size + 10,
      acc_image_y + logo_size - guest_bttn_height - 20,
      logon_width - 30 - (acc_image_x + logo_size + 10) + new_x,
      guest_bttn_height,
    );
    ...
    text(
      "Login as Guest",
      acc_image_x + logo_size + 37,
      acc_image_y + logo_size - guest_bttn_height - 10,
      logon_width - 30 - (acc_image_x + logo_size + 10) + new_x,
      guest_bttn_height,
    );
    ...
    text(
      "Welcome to " + OS_NAME + "!",
      acc_image_x + logo_size + 19,
      acc_image_y + guest_bttn_height / 2,
      logon_width - 30 - (acc_image_x + logo_size + 10) + new_x,
      guest_bttn_height,
    );
    pop();
...
function mousePressed() {
  if (
    mouseX > logon_shift_x + mouse_shift_x &&
    mouseX < logon_shift_x + mouse_shift_x + logon_width
  ) {
    if (
      mouseY > logon_shift_y + mouse_shift_y &&
      mouseY < logon_shift_y + mouse_shift_y + box_top_height
    ) {
...
function mouseClicked() {
  if (
    mouseX > logon_shift_x + mouse_shift_x + logon_width - box_top_height &&
    mouseX < logon_shift_x + mouse_shift_x + logon_width
  ) {
    if (
      mouseY > logon_shift_y + mouse_shift_y &&
  ...
  if (
    acc_image_x + logo_size + 10 < mouseX &&
    logon_width - 30 + new_x > mouseX
  ) {
    if (
      acc_image_y + logo_size - guest_bttn_height - 20 < mouseY &&
      guest_bttn_height + (acc_image_y + logo_size - guest_bttn_height - 20) >
        mouseY
    ) {
      remove();
